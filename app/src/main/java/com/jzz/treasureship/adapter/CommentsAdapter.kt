...
class CommentsAdapter(layoutResId: Int = R.layout.comment_item_layout) :
    BaseBindAdapter<CommentData>(layoutResId, BR.commentData) {
    ...
    override fun convert(helper: BindViewHolder, item: CommentData) {
        ...
        if (item.isLike == 0) {
            //未点赞
            Glide.with(mContext).load(mContext.resources.getDrawable(R.drawable.icon_unlike))
                .into(helper.getView(R.id.iv_praise))
        } else if (item.isLike == 1) {
            //点赞
            Glide.with(mContext).load(mContext.resources.getDrawable(R.drawable.icon_like))
                .into(helper.getView(R.id.iv_praise))
        }
