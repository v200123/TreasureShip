...
import com.jzz.treasureship.ui.goods.GoodsQualificationActivity
 ...
 *@Auth: 29579
 **/
class GoodsDetailsAcvitity  : BaseVMActivity<GoodsDetailViewModel>(false), EasyPermissions.PermissionCallbacks {
...
//        activity!!.nav_view.visibility = View.GONE
        ...
        this.mWindow = this.window

        fab_up.hide()
        initListener()
        tv_cart.setOnClickListener {
            mContext.start<ShopCartActivity> {  }
...
//                .commit()
    ...
    override fun initData() {
        ...
        intent.let {
            goodsId = it.getIntExtra("goodsId",0)
    ...
    override fun startObserve() {
        mViewModel.apply {
            val addCartPopup = XPopup.Builder(mContext).asLoading()
            uiState.observe(this@GoodsDetailsAcvitity, Observer { viewModel ->
            ...
            val xPopup = XPopup.Builder(mContext).asLoading("正在加入购物车。。。")
            addCartState.observe(this@GoodsDetailsAcvitity, Observer {
                if (it.showLoading) {
                    xPopup.show()
                }
            ...
            cartNumUiState.observe(this@GoodsDetailsAcvitity, Observer {
                ...
                it.showSuccess?.let {
                    ...
                    QBadgeView(mContext)
                        ...
                        .setBadgeBackgroundColor(ContextCompat.getColor(mContext,R.color.blue_light))
            ...
            directBuyUiState.observe(this@GoodsDetailsAcvitity, Observer {
                if (it.showLoading) {
                    addCartPopup.show()
                }
            ...
            userState.observe(this@GoodsDetailsAcvitity, Observer {
                it.showSuccess?.let {
                ...
                it.needLogin?.let { needLogin ->
                    if (needLogin) {
                        startActivity(Intent(this@GoodsDetailsAcvitity, LoginActivity::class.java))
                    }
                }
            })
        }
     ...
     fun initListener() {
        fab_up.setOnClickListener {
            slideDetailsLayout.smoothClose(
                true
            )
        }
    ...
    private fun initUi(goodsDetail: GoodsDetail) {
        ...
        if (goodsDetail.goodsType == 1) {
            ...
            val apply = SpannableString("境外商品\t" + goodsDetail.goodsName).apply {
                setSpan(RadiusSpan(Color.parseColor("#FFF0A823"), 9, mContext), 0, 3, Spannable.SPAN_INCLUSIVE_INCLUSIVE)
            }
...
//                                            ).commit()
        ...
        iv_useNote.setOnClickListener {
            ...
            for (ele in goodsDetail.instructionsPictureList) {
                ...
                Glide.with(mContext).load(ele).into(img)
                list.add(img)
            }
            if (list.size > 0) {
